...
FTP.prototype.connect = function(options) {
  ...
  this.options.fileEncoding = options.fileEncoding || 'binary';
...
FTP.prototype.list = function(path, zcomp, cb) {
  ...
  this._pasv(function(err, sock) {
    ...
    source.on('data', function(chunk) {
      buffer += chunk.toString(self.options.fileEncoding);
    });
...
FTP.prototype.nlistSafe = function(path, zcomp, cb) {
  if (typeof path === 'string') {
    var self = this;
    // store current path
    this.pwd(function(err, origpath) {
      ...
      self.cwd(path, function(err) {
        if (err) return cb(err);
        // get dir listing
        self.nlist(zcomp || false, function(err, nlist) {
          // change back to original path
          if (err) return self.cwd(origpath, cb);
          self.cwd(origpath, function(err) {
            if (err) return cb(err);
    ...
    this.nlist(path, zcomp, cb);
};

FTP.prototype.nlist = function(path, zcomp, cb) {
  var self = this, cmd;
  ...
  this._pasv(function(err, sock) {
        ...
        for (var i = 0, len = entries.length; i < len; ++i) {
          ...
          if (parsedVal !== null) {
            if (typeof(parsedVal)=='string') {
              ...
              //if(parsedVal.name && (parsedVal.name!=='.' && parsedVal.name!=='..')) {
                parsed.push(parsedVal);
              //}
            }
          }
    ...
    function gather(entries) {
      var parsed = []

      co(function *(){
        for (var i = 0, len = entries.length; i < len; ++i) {
          var entry = entries[ i ]
          //var path = Path.parse(entry);
          //if (path.name==='.' || path.name==='..') { continue; }
          ...
          try {
            yield new Promise( function(resolve,reject){
              self.size(entry,function(err,ret){
                if(err){
                  parsedVal.type = 'd';
                  reject(err);
                } else {
          ...
          try {
            yield new Promise( function(resolve,reject){
              self.list(entry,function(err,ret){
                if(!err){
                  var _parsedVal = Parser.parseListEntry(ret[0]);
                  parsedVal = _parsedVal;
                }
                resolve();
              });
            });
...
FTP.prototype.mlistSafe = function(path, zcomp, cb) {
  if (typeof path === 'string') {
    var self = this;
    // store current path
    this.pwd(function(err, origpath) {
      ...
      self.cwd(path, function(err) {
        if (err) return cb(err);
        // get dir listing
        self.mlist(zcomp || false, function(err, mlist) {
          // change back to original path
          if (err) return self.cwd(origpath, cb);
          self.cwd(origpath, function(err) {
            if (err) return cb(err);
    ...
    this.mlist(path, zcomp, cb);
};

FTP.prototype.mlist = function(path, zcomp, cb) {
  var self = this, cmd;
  ...
  this._pasv(function(err, sock) {
    ...
    function final() {
      if (done && replies === 2) {
        ...
        var parsed = [];
        for (var i = 0, len = entries.length; i < len; ++i) {
          var parsedVal = Parser.parseMlistEntry(entries[i]);
          if (parsedVal !== null)
            parsed.push(parsedVal);
        }
