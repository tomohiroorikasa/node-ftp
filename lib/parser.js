...
var REX_LISTUNIX = XRegExp.cache('^(?<type>[\\-ld])(?<permission>([\\-r][\\-w][\\-xstT]){3})(?<acl>(\\+))?\\s+(?<inodes>\\d+)\\s+(?<owner>\\S+)\\s+(?<group>\\S+)\\s+(?<size>\\d+)\\s+(?<timestamp>((?<month1>\\w{3})\\s+(?<date1>\\d{1,2})\\s+(?<hour>\\d{1,2}):(?<minute>\\d{2}))|((?<month2>\\w{3})\\s+(?<date2>\\d{1,2})\\s+(?<year>\\d{4})))\\s+(?<name>.+)$'),
    ...
    REX_LISTMLSD = XRegExp.cache('^(?<facts>\\S+)\\s(?<name>.+)$'),
...
Parser.parseMlistEntry = function(line) {
  var ret, info;

  if (ret = XRegExp.exec(line, REX_LISTMLSD)) {
    info = {
      name: ret.name
    };

    facts = ret.facts.split(';');
    facts.forEach(function(fact){
      [key,val] = fact.split('=');
      if (key==='modify') {
        info.date = moment(val," YYYYMMDDHHmmssSS").toDate();
      } else if (key==='perm') {
        //  info. = ;
      } else if (key==='unique') {
        //  info. = ;
      } else if (key==='size') {
        info.size = Number(val);
      } else if (key==='type') {
        if (val==='file') {
          info.type = '-';
        } else if (val==='cdir') {
        ...
        var p = perms.toString(Number('0o'+val));

        info.rights = {
          user: p.substr(0, 3).replace(RE_DASH, ''),
          group: p.substr(3, 3).replace(RE_DASH, ''),
          other: p.substr(6, 3).replace(RE_DASH, '')
        }
        var lastbit = info.rights.other.slice(-1);
        if (lastbit === 't') {
          info.rights.other = info.rights.other.slice(0, -1) + 'x';
